...
fn generate_v_protobuf_encode(pf ProtoFile, node_oneof_fields []ProtoField) {
	...
	for m in sorted {
		...
		vname := proto_field_to_v_type(m.name)
		dfn := snake_case(vname)
		out += 'fn encode_${dfn}(val ${vname}) []u8 {\n'
		out += '\tmut buf := []u8{}\n'
	...
	// Only skip the first variant (Alias) when its encoding is empty,
	// since that is the zero-value default for the sum type.
	// All other variants are always encoded.
	out += 'fn encode_node(val_ Node) []u8 {\n'
	out += '\tmatch val_ {\n'
	for i, f in node_oneof_fields {
		vname := proto_field_to_v_type(f.typ)
		out += '\t\t${vname} {\n'
